<!-- Status: official-read. Source: stable docs/configuration/outbound/http.md, shared/dial.md, and shared/tls.md; reviewed against current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Outbound / http UI Review

## Scope

- Editable node: `outbound:http`
- Official doc: `outbound/http.md`
...
This node writes one object in `outbounds[]` with `type: "http"`.

Official writable fields from `outbound/http.md`:
...
- `path`
- `headers`
- `tls`: outbound TLS object from `shared/tls.md`.
...
- Headers are an object field and need a structured key/value editor.
...
- TLS and headers are Inspector sections, not ports.
...
- `headers` should be a structured key/value repeater, not raw JSON for ordinary users.
- TLS should be an embedded outbound TLS section with advanced certificate fields collapsed.
...
- P1 `headers` needs a structured key/value editor.
- P1 TLS should be available only as outbound TLS, not generic inbound TLS.
