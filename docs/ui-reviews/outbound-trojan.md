<!-- Status: official-read. Source: stable docs/configuration/outbound/trojan.md, shared/dial.md, shared/tls.md, shared/multiplex.md, and shared/v2ray-transport.md; reviewed against current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Outbound / trojan UI Review

## Scope

- Editable node: `outbound:trojan`
- Official doc: `outbound/trojan.md`
...
This node writes one object in `outbounds[]` with `type: "trojan"`.

Official writable fields from `outbound/trojan.md`:
...
- Inspector must expose required `server`, `server_port`, and `password` first.
...
- P0 required server/port/password fields need first-class validation.
