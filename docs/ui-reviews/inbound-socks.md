<!-- Status: official-read. Source: stable docs/configuration/inbound/socks.md and shared/listen.md plus current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Inbound / socks UI Review

## Scope

- Editable node: `inbound:socks`
- Official doc: `inbound/socks.md`
...
This node writes one object in `inbounds[]` with `type: "socks"`.

Official writable fields from `inbound/socks.md`:

- `type`: `socks`
- `tag`
- Listen Fields from `shared/listen.md`.
- `users`: SOCKS users; no authentication required if empty.
...
- SOCKS inbound has no TLS, multiplex, or V2Ray transport fields.
...
- Inspector must expose tag, listen address, listen port, and users.
- `users[]` needs a structured repeater; passwords are sensitive.
- Do not show TLS/multiplex/transport sections for SOCKS inbound.
...
- P0 SOCKS inbound must not inherit TLS/multiplex/transport controls.
