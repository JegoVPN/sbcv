<!-- Status: official-read. Source: stable docs/configuration/inbound/http.md, shared/listen.md, and shared/tls.md; reviewed against current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Inbound / http UI Review

## Scope

- Editable node: `inbound:http`
- Official doc: `inbound/http.md`
...
This node writes one object in `inbounds[]` with `type: "http"`.

Official writable fields from `inbound/http.md`:

- `type`: `http`
- `tag`
- Listen Fields from `shared/listen.md`.
- `users`: HTTP users; no authentication required if empty.
...
- TLS is an embedded inbound TLS section, not a standalone node.
...
- Inspector must expose tag, listen address, listen port, users, TLS, and system proxy toggle.
