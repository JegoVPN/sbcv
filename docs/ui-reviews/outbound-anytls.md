<!-- Status: official-read. Source: stable docs/configuration/outbound/anytls.md, shared/dial.md, and shared/tls.md; reviewed against current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Outbound / anytls UI Review

## Scope

- Editable node: `outbound:anytls`
- Official doc: `outbound/anytls.md`
...
This node writes one object in `outbounds[]` with `type: "anytls"`.

Official writable fields from `outbound/anytls.md`:
...
- `password`: required.
- `idle_session_check_interval`: default `30s`.
- `idle_session_timeout`: default `30s`.
- `min_idle_session`: default `0`.
...
- Inspector must expose required `server`, `server_port`, `password`, and TLS status first.
- `password` should be sensitive.
- Idle session fields should be grouped as advanced session tuning with defaults.
...
- P0 required server/port/password/TLS fields need first-class validation.
- P0 Dial `detour` must use an outbound select/port attachment, not raw tag text.
- P1 idle-session tuning should be grouped away from first-run setup.
