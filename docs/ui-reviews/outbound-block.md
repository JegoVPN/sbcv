<!-- Status: official-read. Source: stable docs/configuration/outbound/block.md plus current Palette, SbcNode, and Inspector registries. UI verification and implementation fixes still required. -->
# Outbound / block UI Review

## Scope

- Editable node: `outbound:block`
- Official doc: `outbound/block.md`
...
This node writes one object in `outbounds[]` with `type: "block"`.

Official writable fields from `outbound/block.md`:
...
- No protocol fields, no Dial Fields, no TLS, no server address, no downstream detour.

Relationship model:

- This outbound can be referenced by `route.final`, route rule `action: "route"`, selector/urltest groups, and any config surface that accepts an outbound tag.
- It has no outbound references of its own.
...
- Right ports: none. `block` has no Dial Fields and cannot chain to another outbound.
...
- Inspector should expose only tag/type and incoming reference summary.
- Do not show server, port, Dial, TLS, transport, multiplex, or detour controls for block.
- Type switching from block to a dialable outbound must deliberately introduce required fields and diagnostics; type switching back to block must clear incompatible protocol fields.
...
- P0 block must not expose generic dial/server/TLS fields.
- P0 right-side downstream ports must be hidden for block.
- P1 Inspector should focus on tag and references only.
