# Outbound VLESS - Codex review cross-map
...
| C1-10 | P1 | Outbound | VLESS `flow` without TLS is valid in upstream/stable, but SBC emits `vless-flow-requires-tls` as an error. | Remove or downgrade that diagnostic; add a stable parity fixture. |
...
SUMMARY: 1 P0, 2 P1, 0 P2; icons 1 P1, 0 P2.
