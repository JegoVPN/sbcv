<!-- Status: official-read. Source: stable + testing docs/configuration/dns/server/fakeip.md, stable + testing dns/fakeip.md (top-level legacy), dns/server/index.md, Palette.tsx, Inspector.tsx, sharedFieldRegistry.ts, protocols.ts, commands.ts, graph.ts, useProjectStore.ts. UI verification + implementation fixes still pending. -->
