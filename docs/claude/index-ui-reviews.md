...
8. **testing-only 字段无 channel gate**：**部分 FIXED 2026-05-27** — `diagnostics.ts` 在 channel === "stable" 时新增 warning：`hysteria2-realm-testing-only` / `hysteria2-bbr-profile-testing-only` / `hysteria2-hop-interval-max-testing-only`、`tun-dns-mode-testing-only` / `tun-dns-address-testing-only` / `tun-mac-address-filter-testing-only`、`ssh-cipher-testing-only` / `ssh-mac-testing-only` / `ssh-kex-algorithm-testing-only`、`dns-server-tailscale-accept-search-domain-testing-only`。整节点门控的 `service:hysteria-realm` 已经渲染 `<PlatformBanner kind="channel">`。**待办**：`cache_file.store_dns`、route 1.14 字段（`find_neighbor` / `dhcp_lease_files` / `default_http_client`）、dns rule 1.14 matchers（`source_mac_address` / `source_hostname` / `preferred_by` / `match_response` / `package_name_regex`）、anytls 节点（1.12.0 版本门控）。

9. **平台限制 UI 不显式**：**部分 FIXED 2026-05-27** — Inspector 现在为 `inbound:redirect`（Linux）、`inbound:tproxy`（Linux）、`inbound:tun`（多平台敏感）、`outbound:tor`（build tag）、`outbound:block`（deprecated）、`outbound:hysteria`（v1 deprecation）、`experimental.v2ray_api`（build tag）渲染 `<PlatformBanner>`，分 platform / build-tag / deprecated / channel 四种语义颜色。**待办**：`dns-server:resolved` + `service:resolved`（Linux/systemd）、`service:hysteria-realm`（1.14 testing channel）、`endpoint-tailscale` + `dns-server-tailscale` + `service-derp`（Tailscale build）、Palette badge / icon 反映 gate、跨 channel 切换时的诊断。
