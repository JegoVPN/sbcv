...
- 节点 review 数：**66 / 66**
- Status：63 / 66 `official-read`，2 / 66 `ui-verified`（`outbound:selector`、`outbound:urltest`，2026-05-27 部分 P0 修），1 / 66 `implemented`（`settings:log`，2026-05-27）。
...
2. **共享字段表大面积缺失字段**：
   - `dialSharedFields` 缺 12+ 个官方字段（`inet4_bind_address`、`inet6_bind_address`、`bind_address_no_port`、`routing_mark`、`reuse_addr`、`netns`、`tcp_fast_open`、`tcp_multi_path`、`disable_tcp_keep_alive`、`tcp_keep_alive`、`tcp_keep_alive_interval`、`udp_fragment`、`domain_strategy`）。所有 outbound / dns-server / NTP / endpoint 都受影响。
   - `listenSharedFields` 缺 5–6 个（`tcp_multi_path`、`disable_tcp_keep_alive`、`tcp_keep_alive`、`tcp_keep_alive_interval`、`udp_fragment`、listen `detour`）。所有 inbound + service-derp / resolved / hysteria-realm 都受影响。
   - `tlsSharedFields` 缺服务端必需字段（`key_path`、内联 PEM）和高级字段（Reality 子对象、uTLS、ECH、`record_fragment`、`fragment_fallback_delay`）。受影响包括所有 server-side TLS 节点（trojan / vless / vmess inbound、hysteria 系列、DoT/DoQ/DoH/DoH3、derp、hysteria-realm）和所有 client-side Reality outbound（vless）。
