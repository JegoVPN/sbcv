...
- `ui-verified`: 65 / 66 — every other node sits on the shared atomics (dial/listen/TLS shared fields with gated Reality / uTLS / ECH sub-fields, JsonField fallback, structured users / peers / hosts / CCM/OCM / hysteria-realm / derp verify_client_url + mesh_with + stun / TUN platform.http_proxy + route_address* / shadowsocks SIP003 plugin / hysteria2 obfs + server_ports + hop_interval / TUIC udp_relay_mode / tor torrc + extra_args / naive extra_headers / urltest url+interval+tolerance+idle_timeout editors, TLS-required scaffold, sensitive masking, action-gated rule sub-forms, platform / channel / build-tag banners on redirect / tproxy / tun / tor / block / hysteria v1 / v2ray_api / resolved / hysteria-realm / tailscale endpoint + dns + derp, domain_resolver tag select, cache_file 1.13/1.14 fields, Clash API external_ui controls, outbound enum selects, route/dns hub fields, selector default cascade, Reality + xtls-rprx-vision + vmess/vless/tuic credential + tuic UDP conflict + hysteria2 server_port vs server_ports + 24 channel-gated testing-only-on-stable warnings + 3 deprecation warnings + DERP config_path + NTP server + urltest URL diagnostics, rule-set/dns-server diagnostics, Palette `deprecated` status for block / hysteria v1 / fakeip).
- `official-read`: 0 / 66.
- `baseline`: 0 / 66.
