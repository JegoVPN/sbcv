...
- [x] U9-dns-optimistic-object — `optimistic` 复合控件（enabled + stale-serve window `timeout`），并澄清相邻 per-query Timeout 标签。Touch: `src/components/inspector/dnsInspector.tsx`. **DONE (PR #290).**
- [x] U10-domain-resolver-empty-state — `domain_resolver`/`default_domain_resolver` select 加 hint +（可选）就地创建 DNS server。Touch: `src/components/inspector/sharedFields.tsx`. **DONE (PR #291;就地创建按 spec 可选,deferred)。**
- [x] U11-inline-http-client — `http_client` 内联对象入口（tag→object 切换）+ diagnostic 校验内联对象里的不支持 key。Touch: `sharedFields.tsx`, `src/domain/diagnostics.ts`. **DONE (PR #292).**
- [x] U12-cert-provider-acme — standalone ACME certificate-provider 的 `dns01_challenge` 结构化编辑器 + `http_client` select。Touch: `sharedFields.tsx`, `handledFields.ts`, `certificateProviderInspector.tsx`, `Inspector.tsx`.（与 C2/C3 cross-ref）**DONE (PR #293).**

#### Phase U-P3 — polish / hardening + re-assessment
- [x] U13-rule-action-scrub — 扩展 `normalizeRouteRule` 的 drop 列表（sniffer/timeout/strategy/route-options 组），修测试注释。Touch: `src/domain/commands.ts`, `tests/rule-action-field-scrub.test.ts`. **DONE (PR #294).**
- [x] U14-deprecation-hardening — `download_detour` 1.16 升 error + both-set 冲突 diagnostic；testing 上 Download Detour 控件按值/渠道门控；修正 version-gate 注释。Touch: `src/domain/diagnostics.ts`, `ruleSetInspector.tsx`, `versionFieldGate.ts`. **DONE (PR #296;可选 SINGLETON_GATE_SECTIONS 表 deferred)。**
- [x] U15-network-enum-derp — `network` matcher enum(tcp/udp/icmp) + 修内联标签；DERP `verify_client_url` detour→outbound `<select>` + 余下 HTTP Client Fields。Touch: `ruleControls.tsx`, `serviceInspector.tsx`. **DONE (PR #297 U15a + #298 U15b).**
...
- **U15 (PR #297 U15a + #298 U15b)** — network enum 与 DERP,按 spec 拆两 slice。**U15a (#297):** route/DNS rule `network` matcher 从自由文本改为固定 tcp/udp/icmp 复选框组(RuleAdvancedFields 新增 `field === "network"` 分支,杜绝 typo;仍为 list);修内联标签「Network (tcp/udp)」→「(tcp/udp/icmp)」。**U15b (#298):** DERP `verify_client_url` 行 detour 从自由文本改 outbound `<select>`(outboundTags,仿 ccm/ocm)+ per-row JSON 编辑器暴露余下 HTTP Client Fields(tls/headers/dial,只编辑 url/detour 之外的 key 并合并回 url/detour,不重复)。迁移既有 app.test DERP 测试到 select。**U15b reviewer REQUEST-CHANGES → 修复:** verify_client_url 行可为字符串简写(`"__URL__"` ≡ `{url}`,derp.md:67-71),解构字符串会展开成字符索引垃圾且编辑时丢 URL;在 rows 派生处把字符串行规范化为 `{url}`(仿 verify_client_endpoint),加回归测试。均无序列化改动。Reviewer 两 PR 均**隔离 worktree** 评审。Tests: `tests/network-enum.test.tsx`, `tests/derp-verify-client-url.test.tsx`。Verify: tsc clean, 1668 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success（两次)。
- **U14 (PR #296)** — download_detour 弃用硬化。Diagnostic 版本升级:1.14/1.15 warn(`rule-set-download-detour-deprecated`)、≥1.16 error(`rule-set-download-detour-removed`,字段已移除);从 `channel === "testing"` 改 version-based `atLeast`(stable 1.13 仍 clean,行为对三个目标不变,仅新增 ≥1.16 升级——前瞻,validateConfig 可传 version="1.16" 测)。新 both-set 冲突 warn(`rule-set-download-detour-http-client-conflict`,http_client 优先,download_detour 冗余)。UI:testing 上 Download Detour 控件仅当已有值才渲染(从零项目被导向 http_client,配对 U1;stable 始终显示)。修正 VT3 注释过度声称(实际仅覆盖 7 个 array-collection kinds 顶层字段;singleton/nested 手工门控)。可选 SINGLETON_GATE_SECTIONS 表 deferred。无序列化改动。Reviewer **APPROVE(隔离 worktree** —— 修复上轮 #294 共享工作树污染教训,见记忆 [[feedback_reviewer_subagent_no_shared_git]]):版本升级/both-set/UI 门控/注释准确性全验证。Tests: `tests/download-detour-gating.test.tsx`。Verify: tsc clean, 1662 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。devlog 折入本 PR。
- **U13 (PR #294)** — 扩展 `normalizeRouteRule` 的 action-specific 字段 scrub:sniff-only `sniffer`;resolve-only `strategy`/`disable_cache`/`disable_optimistic_cache`/`rewrite_ttl`/`client_subnet`(U6a 旋钮);route-options 组(override_*/network_strategy/fallback_*/udp_*/tls_*)离开 route/route-options/bypass 时 scrub。**两处上游修正:** (1) `timeout` 是 sniff **与** resolve 共用(U6a 加了 resolve query timeout),故仅 action ∉ {sniff,resolve} 才 scrub——spec 字面「!=="sniff" 时 drop timeout」会丢 resolve.timeout;(2) `network_type` 是 **MATCH 字段**(每个 action 有效),刻意排除出 route-options 组(`network_*` glob 会致数据丢失)。仅 import/edit 边界 normalization,无 per-field 序列化。**normalizeDnsRule 未动**(其 action 集已完整)。Reviewer APPROVE(逐字段对照 rule.md/rule_action.md 核验无 over-scrub)。Tests: 扩展 `tests/rule-action-field-scrub.test.ts`(各组 positive+keep-on-owner negative + network_type guard + timeout-on-both);改过时注释。**全量无 over-scrub 回归**。Verify: tsc clean, 1654 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。devlog 折入本 PR。
- **U12 (PR #293)** — standalone ACME cert-provider 的 `dns01_challenge` 结构化编辑器 + `http_client`。抽 `dns01ChallengeFields(root, channel)` 共享 helper(inline-TLS root `tls.acme.dns01_challenge` 与 standalone root `dns01_challenge` 共用,**byte-identical refactor**,reviewer 逐字核验),provider 凭据 visibleWhen 门控(alidns/cloudflare/acmedns),1.14 字段(ttl/propagation_*/resolvers/override_domain)channel-gate。`certificateProviderFields` 加 channel 参,acme 分支 append dns01 editor + HTTP Client select(复用 U1 createKind + U11 objectForm)。导出 `isSharedFieldVisible(definition, entity)`(原 inline 在共享组渲染器里)供 cert-provider inspector 复用(dns01 visibleWhen 依赖它);去重 gatedBy/visibleWhen 逻辑。channel 从 shell 线入 CertificateProviderInspector。dns01_challenge/http_client 入 certificateProviderHandledFields。无序列化改动。Reviewer APPROVE 零 actionable(refactor 等价 + dns01 契约 + http_client reachability via updateEntityField cert-provider 分支均确认)。Tests: `tests/cert-provider-acme.test.tsx`。Verify: tsc clean, 1649 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。devlog 折入本 PR。
- **U11 (PR #292)** — 内联 http_client。**Diagnostic(优先):** 新 `http-client-unsupported-field`(warning,跨渠道):内联 http_client 对象含 HTTP Client adapter 不支持的 key 时告警——顶层(version/disable_version_fallback + HTTP2/QUIC fields:idle_timeout/keep_alive_period/stream_receive_window/connection_receive_window/max_concurrent_streams/initial_packet_size/disable_path_mtu_discovery)或 tls.*(alpn/ech/utls/reality/cipher_suites/fragment/client_certificate* 等);精确等值匹配(无前缀 bug,`tls.certificate` 等 supported key 不误报);覆盖 route.default_http_client + rule_set[].http_client。**UI:** 拓宽 `objectForm` 含 "http-client",http_client/default_http_client select 加「Add inline configuration」按钮(seed `{}` → 既有 JsonField 分支渲染编辑器)。无序列化改动。**Reviewer APPROVE + 1 actionable(已应用):** 初版 unsupported 顶层集漏了 HTTP2/QUIC fields,补全;另加 tls.certificate not-flagged 回归测试守护前缀 bug。**遗留 owner(reviewer nit#2,记此备忘):** cert-provider http_client(→U12)、hysteria2 `realm.http_client`、DERP `verify_client_url` HTTP Client Fields、Tailscale `control_http_client` 的内联对象暂未纳入该 diagnostic。Tests: `tests/http-client-inline.test.tsx`。Verify: tsc clean, 1643 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。devlog 折入本 PR。
- **U10 (PR #291)** — domain_resolver 空状态 hint(与 U1 同类):`domainResolverHint` 基于 `dnsServerOptions.length`——无 DNS server 时显示「add a DNS server first」空状态提示,否则显示引用提示;挂到 route `default_domain_resolver` 与 dial-group `domain_resolver` 两处(镜像 U1 httpClientHint 模式)。就地创建 DNS server(U1 createKind 模式)按 spec 为可选,**deferred**——hint 已消除死胡同(该项 P2,既有 diagnostic 已软化)。无序列化改动。Tests: `tests/domain-resolver-empty-state.test.tsx`。Reviewer APPROVE 零 actionable。devlog 折入本 PR。Verify: tsc clean, 1637 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。
- **U9 (PR #290)** — DNS `optimistic` 从纯 bool 改为复合控件:enabled checkbox + 条件「Stale-serve window」text;有 window 写对象 `{enabled:true, timeout}`,仅 enabled 收敛为 bool `true`(上游默认 3d),off→undefined,清空 window→collapse 回 bool。相邻 per-query `dns.timeout`(10s)重标为「Query Timeout」以消歧(它与 optimistic window 无关)。optimistic/timeout 保留在 dnsHandledFields。无序列化改动。Reviewer APPROVE(确认从不写半成品对象 `{enabled:true}`/`{timeout}`/`{enabled:false}`;既有 app.test DNS-hub 测试不回归);采纳其 nit 把局部 `window` 改名 `staleWindow`(避免遮蔽全局)。Tests: `tests/dns-optimistic.test.tsx`(bool/对象/collapse/导入往返/window 门控/Query Timeout 标签)。Verify: tsc clean, 1634 tests green, build clean, `pnpm test:binaries` 19/19, Workers Builds success。devlog 折入本 PR。
