...
- 2026-08-23 — Atomic 3 production rollout: Cloudflare Workers Builds completed successfully for `main` at `fc156ed`. The production site loaded the same JavaScript asset as the locally accepted build (matching SHA-256), and the live UI switched to `1.14 testing` and created a Remote Rule Set successfully. Verification reused the user's signed-in Chrome session; temporary dashboard and smoke tabs were closed afterward.
- 2026-08-23 — Atomic 4a added the bounded scalar-field slice: stable AnyTLS `client_metadata` with a 1.12 export gate, plus testing Tailscale `listen_port` / `taildrop_directory`, Hysteria2 `disable_chrome_parrot`, and remote Rule Set `initial_path`. UDP NAT remains a separate shared-field atomic so this change does not mix cross-parent schema work into the simple Inspector controls.
- 2026-08-23 — Atomic 4a checks passed: direct checks with all three pinned binaries, `git diff --check`, TypeScript and production build, 1,885 Vitest cases across 193 files with a 30-second per-test ceiling, 121-doc coverage audit, 22 runnable target-matched fixture checks (1 Linux-only namespace fixture skipped on macOS), 24 export/binary tests, 42 official external checks plus 237 accepted external render/import fixtures across 30 isolated shards, and 43 Playwright E2E cases at four workers. The default five-second run hit the same existing fixture mutation coverage timeout; that file passed in the focused run and the complete 30-second-ceiling run passed.
- 2026-08-23 — Frontend performance review for Atomic 4a: the controls edit existing canonical objects through the existing narrow Inspector command path. They add no dependency, lazy boundary, store subscription, graph derivation, async waterfall, or transient canvas state. The testing-only Rule Set field is hidden on a fresh stable target but remains reachable when imported; the production build retains only its pre-existing large-chunk warning.
- 2026-08-23 — Atomic 4b adds the shared UDP NAT field group to the already-creatable TUN, TProxy, and WireGuard parents on `1.14 testing`. The three new fields use one registry-owned enum/schema definition, canonical Inspector updates, channel-aware handled-field fallbacks, stable export gates, uint32-shape validation, and a target-matched fixture. OpenConnect/OpenVPN inherit this group only when their larger Endpoint objects land, keeping this atomic bounded.
- 2026-08-23 — Atomic 4b checks passed: direct stable/testing stdin probes for accepted values and rejected stable/invalid shapes, `git diff --check`, TypeScript and production build, 1,889 serial Vitest cases across 194 files, 121-doc coverage audit, 23 runnable target-matched fixture checks (1 Linux-only namespace fixture skipped on macOS), 25 export/binary tests, 42 official external checks plus 237 accepted external render/import fixtures across 30 isolated shards, and 43 Playwright E2E cases at four workers. The first serial run found one expected stale capability assertion for WireGuard's new testing group; the focused rerun and complete serial rerun passed after updating it.
- 2026-08-23 — Frontend performance review for Atomic 4b: the UI reuses the existing shared-card renderer and passes the already-selected channel into the two family inspectors. It adds no dependency, store subscription, graph derivation, async work, or transient canvas state; channel switching changes only the shared-group and fallback field sets already owned by the selected Inspector.
- 2026-08-23 — Atomic 4c adds the testing-only DNS query match fields `query_client_subnet`, `query_dnssec`, `package_name_regex`, and `preferred_by` to the DNS Rule Inspector. Stable keeps fresh controls hidden while imported values remain editable and export-blocking. `preferred_by` is modeled as a DNS-server tag list, including recursive rename/delete cascades and missing-reference diagnostics for logical sub-rules.
- 2026-08-23 — Atomic 4c checks passed: direct stable/testing stdin probes, `git diff --check`, TypeScript and production build, the complete serial Vitest suite, 121-doc coverage audit, 24 runnable target-matched fixture checks (1 Linux-only namespace fixture skipped on macOS), 26 export/binary tests, 42 official external checks plus 237 accepted external render/import fixtures across 30 isolated shards, and 43 Playwright E2E cases. The action-option fields `disable_optimistic_cache`, `timeout`, and `remove_client_subnet` remain the next bounded DNS atomic.
- 2026-08-23 — Frontend performance review for Atomic 4c: the four controls reuse the selected DNS rule Inspector and existing canonical update command. They add no dependency, store subscription, graph derivation, async work, data waterfall, or transient canvas state; the only recursive work is the existing tag rename/delete event path, linear in DNS rule depth.
- 2026-08-23 — Atomic 4c production rollout: Cloudflare Workers Builds completed successfully for `main` at `c6cc9cc`, and the resulting version received 100% of production traffic. The live editor exposed all four query match controls on `1.14 testing`; verification reused the signed-in Chrome session, restored `1.13 stable`, and closed the temporary tabs afterward.
- 2026-08-23 — Atomic 4d adds DNS query action options to the selected-rule Inspector. Stable exposes `disable_cache`, `rewrite_ttl`, and `client_subnet`; testing additionally exposes `disable_optimistic_cache`, `timeout`, and `remove_client_subnet`, while imported testing values remain reachable and export-blocking on stable. Canonical normalization removes the six options when an incompatible action is selected, `rewrite_ttl` is constrained to uint32, and the two client-subnet modes are mutually exclusive in both the UI and semantic validation.
- 2026-08-23 — Atomic 4d checks passed: direct stable/testing stdin probes, `git diff --check`, TypeScript and production build, 1,898 serial Vitest cases across 196 files, 121-doc coverage audit, 25 runnable target-matched fixture checks (1 Linux-only namespace fixture skipped on macOS), 27 export/binary tests, 42 official external checks plus 237 accepted external render/import fixtures across 30 isolated shards, and 43 Playwright E2E cases. An initial full suite run overlapped the external fixture shards and hit one existing lazy-palette wait timeout; the case passed alone in 0.9 seconds and the complete resource-isolated rerun passed.
- 2026-08-23 — Frontend performance review for Atomic 4d: the controls reuse the selected DNS rule Inspector and canonical update command, without adding a dependency, config-store subscription, graph derivation, async work, data waterfall, or transient canvas state. Action normalization is constant work over six optional fields, and the production build retains only its pre-existing large-chunk warning.
- 2026-08-23 — Target-label follow-up exposes the exact official-checker release in both desktop and mobile selectors (`1.12.25`, `1.13.19`, and `1.14.0-beta.17`). Channel IDs and API labels remain unchanged; a unit guard keeps the displayed releases aligned with both local installer and production container pins. The change adds only static registry strings and no dependency, store subscription, derived graph work, or async path.
- 2026-08-23 — Target-label checks passed: `git diff --check`, TypeScript and production build, 1,900 serial Vitest cases across 196 files, 43 Playwright E2E cases, and signed-in Chrome visual/interaction QA with no relevant console output or layout clipping. Binary fixture checks were not rerun because this display-only atomic does not change emitted config or any binary artifact.

## Notes And Deviations

- 2026-08-23 — The product still targets minor channels internally, but the user-facing selector now includes each concrete validator pin so users can see which official binary performs validation.
