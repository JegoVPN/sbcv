...
#### A12-dial-fields-migration-gate — ⚠️ **REVISED 2026-06-01：premise 被二进制推翻 → 被 A8 涵盖（无独立 gate）**
- **原 Outcome（已撤回）:** ~~新增配置级检查：`direct` outbound + 域名 DNS 服务器 + 无 default → 版本门；「即便全 per-server 解析仍 FATAL」。~~
- **实际 Outcome（binary-grounded）:** **不新增任何 config-level gate。** 二进制三版重放推翻了「distinct gate」premise——`gate-perserver-only-no-default` 的 1.13+ FATAL **纯因 `direct` outbound 自身缺 resolver**（= A8 的精确条件），与域名 DNS 服务器是否 per-server 解析**无关**；`A12-cover`(给该 direct 加 `domain_resolver`)三版 **PASS**。故 spec「即便 per-server 全满足仍 FATAL」**为假**。A8（已落地）已精确覆盖 gate seed（direct + ≥2 DNS 无 resolver → 1.12 warn/1.13+ error）。本原子项 = **回归锁定 + spec 更正**(无新代码)。
- **推翻依据（二进制 + validateConfig 双验）:** gate-seed → 1.12 warning/1.13 error(A8 触发,匹配二进制);A12-cover(direct 带 resolver)→ 三版 silent(二进制 PASS);§Coverage-4 四个非触发边界(仅域名 DNS 无 outbound / direct 无 dns / +block / +socks-IP)→ 三版 silent。**不另设宽门即避免 #303 redux**(过宽 config-level gate 会在这些边界假阳性)。
- **Source of truth:** `shared/dial.md`（direct → "Domain in request" = dial 字段,与 outbound 同门）；二进制 `ENABLE_DEPRECATED_MISSING_DOMAIN_RESOLVER`。
- **Touch:** **无代码改动。** 仅 `tests/domain.test.ts` 新增 A12 describe(gate-seed 由 A8 触发 + A12-cover silent + 4 非触发边界 silent)+ 更正本章节。
- **Acceptance:** gate-seed 1.12 warn/1.13+ error(经 A8);A12-cover + 4 边界 三版 silent。
- **Reviewer:** version-gating/diagnostics（复核 A8 确实涵盖 gate seed 且未对边界过度报警）。
- **Don't mix:** 仅锁定 A8 涵盖现状 + spec 更正；不新增 gate。
...
- **2026-06-01 — A11 落地（本 PR, L-P1 先落）⚠️#303。** 新增 `missing-default-domain-resolver`(error,`/route/default_domain_resolver`):当 default 命名不存在的 DNS server tag 且存在域名消费者时报错。二进制全套取证(E1–E7 + 6 个种子):悬空 default + 域名/`direct` outbound 或 tailscale/wireguard-domain-peer endpoint → 三版 `check` FATAL `default domain resolver not found`;仅 IP outbound(无消费者)→ 三版 PASS(故门控消费者);单 DNS server **不救**悬空 default(E6 FATAL);string + `{server}` 两形式;消费者**不含** DNS server(E1 PASS,域名 DNS server 用 per-server resolver,归 A6)。**implicit-cover 设计决策(偏离 spec 字面"收紧"):** 二进制对悬空 default 仅发**一条**错误,故采"非双报"——用精确的 A11 error 取代"取消屏蔽通用 per-entity 警告"(后者会双报且文案误导"default is unset");per-entity 屏蔽语义经 `defaultDomainResolverPresent` 保留不变;#303 的 DNS-server 臂由 A6 移除。crosscheck:`default-domain-resolver-dangling` 族 6 case 全部 validateConfig↔三二进制 **0 divergence**。W7 parity:`missing-default-domain-resolver` 加入 dns-server `staleDiagnosticCodes`(删除 local-dns 触发)。**reviewer 抓到 FP-1(#303 形状假阳性,BLOCKING):消费者门忽略 per-entity `domain_resolver` override——实体自带 resolver 时不消费 default,悬空 default 无人消费 → 二进制 PASS,但我方报错。已二进制独立确认(outbound/endpoint 自带 resolver + 悬空 default → 三版 check 0)并修复(门排除 `resolverPresent(domain_resolver)` 实体,镜像 per-outbound 早返回)+ 加 4 个 override 回归 + crosscheck override 种子 0 divergence。**
- **2026-06-01 — A6 落地（本 PR, L-P1 后落）⚠️#303。** `dns-server-domain-without-resolver` warning→**error**,移除 DNS-server 的 `domainResolverImplicitlyCovered` 逃逸(line 1909),改文案(原文案三处皆误:default 无关、"more than one DNS server" 无关、"1.14+" 错——1.12 即 FATAL)。二进制三版 `check` FATAL `missing domain resolver for domain server address`:`fn-default-resolver-no-perserver`(有 default 无 per-server)、`fn-single-server-domain-no-resolver`(单域名 DNS server)均 reject;加 per-server resolver → PASS;IP 字面量 server → PASS(单服务器可选性仍救 IP)。**关键复证:case A 的精确配置(#303 触发配置:RemoteDNS=cloudflare-dns.com 无 per-server + default=LocalDNS)三版 check FATAL** —— #303 修对了 outbound(default 覆盖 outbound)但错套到 DNS server。测试:迁移 `tests/domain.test.ts` case A/B 的 dns-server 断言(`not.toContain`→error,被二进制推翻的旧断言)+ 新增 A6 describe(default/single → error;per-server/IP → silent)。crosscheck:`dns-server-resolver` 族 **11/11 agree**(无假阳性)。
- **2026-06-01 — ⚠️ A12 预警(二进制推翻 distinct-gate premise)。** 边界测试:`gate-perserver-only-no-default`(direct 无 per-outbound resolver)1.13+ FATAL = **正是 A8 条件**;`A12-cover`(direct **带** `domain_resolver`)三版 PASS。故 spec「即便 per-server 全满足仍 FATAL」被推翻——A12 **≡ A8**(被涵盖)。非触发边界 b1–b4 全 PASS(印证 §Coverage 4)。到 A12 时确认 A8 覆盖后判定 subsumed,不另设易假阳 config-level gate。
- **2026-06-01 — A6 reviewer = APPROVE（PR #311 merged）。** 独立 `check` 重放四态:#303 触发配置/单域名 DNS server 三版 FATAL=error✓;per-server resolver / IP 字面量 三版 PASS=silent✓。无 #303 redux,无假阳性。
- **2026-06-01 — A7+A8 落地（PR #312, merged）。** reviewer = APPROVE(独立 check exit 重放五态全匹配,含 0-DNS FP 修复;FP-hunt 无新假阳性)。两 cosmetic nit(stale 标题、channel/version),标题已在 A9/A10 PR 顺带修。
- **2026-06-01 — A9+A10 落地（PR #313, merged;L-P2;共用 endpoints 遍历,分别测)。** reviewer = APPROVE(独立 11 case check 重放全匹配:A10 含 1.12 无条件 error、A9 版本门、FP-hunt 全静默、malformed 输入健壮)。**A12 落地（本 PR, L-P2 收尾)= ⚠️ 反转。** 二进制推翻 A12 distinct-gate premise:gate seed 1.13+ FATAL 纯因 direct outbound 缺 resolver(A8 条件),A12-cover(direct 带 resolver)三版 PASS → A8 已精确涵盖。无新 config-level gate(避免 #303 redux)。本 PR 仅加 A12 describe(gate-seed 经 A8 触发 1.12 warn/1.13+ error + A12-cover silent + §Coverage-4 四边界 silent)+ 更正 spec A12 章节。A12 reviewer = APPROVE(三角验证 tri_a PASS/tri_b FATAL 证 FATAL 源是 direct+≥2DNS;A8+A6 联合覆盖,无 false negative)。
- **2026-06-01 — A13 落地（本 PR, L-P3,生成器层）。** W9 unknown-field linter 此前对所有 rule-set 跳过(`knownFieldsFor('rule-set',type)` 返回 null,因生成器排除 index.md,而 rule-set 类型 remote/local/inline 的字段只在 index.md)。修在**生成器** `scripts/gen-known-fields.mjs`:特例化 rule-set,从 `rule-set/index.md` 提取 `#### field` **并集**赋给 remote/local/inline(并集而非按段,避免误报合法 url/download_detour;testing 自动含 http_client),重新生成 `knownFields.generated.ts`(+63 行)。二进制:伪造字段 `totally_made_up_field` 三版 FATAL `json: unknown field`;合法 remote(url/download_detour/update_interval)/local(path)/inline(rules)/testing http_client 三版/对应版本 PASS。测试:bogus → unknown-field error;四类合法形式 → 不误报。**连带**:VT3 testing-only-field 衍生目录快照更新(rule-set http_client 现被正确识别为 testing-only,`vitest -u`,仅此新增)。`pnpm test` 1715 全绿。 新增 endpoints resolver 检查(此前 endpoints 整类无 resolver 检查)。**A9 wireguard 域名 peer**:dial-field 弃用门,severity `atLeast(version,"1.13")?error:warning`(code `endpoint-domain-without-resolver`)。**A10 tailscale**:始终解析 control_url(controlplane.tailscale.com),**无条件 error**(1.12 也 FATAL,code `endpoint-tailscale-without-resolver`)。共用门 = `resolverPresent(endpoint.domain_resolver) || domainResolverImplicitlyCovered`(default 或 ≤1 DNS)。二进制重放:`fn-endpoint-tailscale-noresolver` 三版 FATAL(含 1.12);`fn-endpoint-wg-domain-multi-dns-noresolver` 1.12 WARN/1.13+ FATAL;clean controls(per-endpoint/default/单DNS/0DNS — E10 tailscale+0dns PASS)+ wg IP peer 三版 PASS。crosscheck:`outbound-endpoint-resolver` 族 **16/16 agree**。**A7/A8 的 cosmetic nit(#303 测试标题)顺带修正。** `outbound-domain-without-resolver` severity 由恒 warning 改为 `atLeast(version,"1.13")?error:warning`(A7,镜像 dns-server-legacy 版本键控)+ 重写版本感知文案;扩展到 `direct` outbound(A8:无 `server` 字段但解析请求域名)。**附带修复**:触发条件由 `singleDnsServerConfigured(===1)` 改为 `atMostOneDnsServer(<=1)`——零 DNS server 时二进制三版 PASS(domain outbound 与 direct 皆是),但旧代码在 domain+0dns 上恒发 warning(**pre-existing 假阳性**),A7 版本门会将其升为假阳性 error,故必须修正门为 ≥2 DNS。二进制重放:`fn-outbound-domain-multi-dns-noresolver`/`fn-direct-multi-dns-noresolver` 1.12 WARN(exit0)/1.13+ FATAL(exit1)=版本门匹配;0-DNS/单-DNS/default/per-outbound resolver 三版 PASS=silent。crosscheck:`outbound-endpoint-resolver`(16)+`dial-fields-deprecation-gate`(10)两族**全 agree**(含 clean-zero-dns control)。`domainResolverImplicitlyCovered` 此时仅 outbound 检查使用(A6 已移除 dns-server 用法),改门不影响他处。测试:迁移 case A-E 不破 + 新增 A7/A8 describe(版本门三态 + direct + 0-DNS fix + 覆盖态静默)。
- **2026-06-01 — A5 落地（PR #309, merged）= ⚠️ 反转。** 三-binary 三-control `run` 重放推翻 spec「降级」premise:tailscale DNS-server `endpoint` 在 run/init 解析且 FATAL(`ts-fp-dangling-endpoint` / `ts-fp-endpoint-wrong-namespace` 三版 `run` FATAL `endpoint not found`;`ts-clean-control` 三版 `run` started 仅 TRACE = 隔离证明;空/缺失 三版 `run` FATAL `missing tailscale endpoint tag`)。= 运行时真阳性(同 `route.final`)→ **保 error,无代码改动**。原审计「run 干净」误植 clean-control 行为。本 PR 仅加锁定测试 + 更正 spec A5 章节,并标注 A17 连带作废。两次独立 reviewer(A2/A3 reviewer 顺带)亦确认 run-FATAL,共三次印证。
