...
### A8b-brands node-brand-svgs — real WireGuard/Tor/Tailscale logos (canvas, A8b follow-up)
Status: implemented 2026-05-29 in `atomic/node-icon-brands`; merged in PR #86. Unblocked by the user
(the confirmed brand SVGs already ship in the project's own v4 icon preview; "直接用").

- What changed: the three brand node-identity icons (outbound/tor, endpoint|outbound/wireguard,
  endpoint|dns-server/tailscale) rendered interim 2-letter monograms (`mono:TO/WG/TS`) pending a
  brand-asset decision. They now render the real logos. New `src/canvas/brandIcons.tsx` carries the
  single-path, `fill="currentColor"` brand glyphs **extracted verbatim** from
  `docs/ui-reviews-pass2/_icons-preview-v4.html` (the same confirmed v4 set the rest of the registry
  already follows); `nodeIconId` now returns `brand:tor` / `brand:wireguard` / `brand:tailscale`, and
  `getNodeIcon` resolves those through a `BRAND_RENDERERS` map. Collision-free guarantee preserved
  (brand ids are distinct; wireguard/tailscale legitimately shared across their two kinds).
- Cross-check (per user request "核对一下所有 svg 用的都是这个 html 里的"): scripted a comparison of all
  54 node-identity icons in the v4 HTML against the registry's resolvable id universe — every one
  resolves (proxy/DNS monograms, inbound/outbound/dns-server/service functional glyphs, hubs/rules/
  settings, and the 3 new brands). Nothing foreign in the registry, nothing from the HTML missing. The
  brand path data is byte-for-byte from the HTML (generated by script, not hand-transcribed).
- Tests: updated `tests/icon-registry.test.tsx` — the 6 brand id expectations (`mono:*`→`brand:*`) plus
  a new render test asserting tor/wireguard/tailscale produce a path-based `svg.brand-icon--<name>` and
  not a `[data-monogram]`.
- Expert review (one pass): a senior reviewer subagent. Verdict APPROVE, no blockers, no should-fix,
  no nits. Independently confirmed the verbatim extraction (byte-for-byte: wireguard 2319 / tor 2527 /
  tailscale 517 chars, viewBox `0 0 24 24`), the `getNodeIcon` brand-first resolution order under
  noUncheckedIndexedAccess, the preserved collision guarantee, structural type-compat (no import cycle),
  and that no `.brand-icon` CSS is needed — every consumer passes an explicit `size`, and
  `fill="currentColor"` inherits the same muted icon tint (`.sbc-node-titlebar__icon` color) as the
  Lucide stroke glyphs. Bundle +~5KB, licensing fine (user-authorized, in-repo confirmed preview).
