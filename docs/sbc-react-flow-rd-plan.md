...
- 以 SagerNet/sing-box `testing/docs/configuration` 和 sing-box 官方配置文档作为字段、版本、废弃状态的核实来源。
...
1. 首要来源：SagerNet/sing-box 仓库 `testing` 分支的 `docs/configuration` 目录。
2. 稳定公开来源：sing-box 官方配置文档站点。
3. 最终行为来源：本地或服务端执行 `sing-box check`。

已经核实的关键事实：

- `docs/configuration/index.md` 明确 sing-box 使用 JSON 作为配置文件格式。
- 顶层配置包括 `log`、`dns`、`ntp`、`certificate`、`certificate_providers`、`http_clients`、`endpoints`、`inbounds`、`outbounds`、`route`、`services`、`experimental`。
