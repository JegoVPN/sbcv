...
| outbound-anytls | `idle_session_check_interval` / `idle_session_timeout` / `min_idle_session` missing from `outboundHandledFields`. ✅ 1.12+ version gate is N/A — anytls is supported by every target we ship. |
