...
| outbound-hysteria | `up_mbps` / `down_mbps` not in `outboundHandledFields`. ✅ ~~`hysteria-v1-deprecated` diagnostic missing~~ — fixed 2026-05-27; `outbound-missing-tls` already covers TLS-required. obfs UI only in hysteria2 branch — not in hysteria. |
