...
| inbound-anytls | **`padding_scheme[]` completely invisible** (not in `inboundHandledFields`). ✅ `address`/`auto_route` fixed under CC-1. ✅ 1.12+ version gate is N/A — anytls (1.12+) is supported by every target we ship (`1.12-stable`, `1.13-stable`, `1.14-testing`); no gate needed. |
