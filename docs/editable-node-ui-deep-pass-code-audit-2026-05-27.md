...
| service-derp | **`commands.ts:501` scaffold returns 8 fields with no `tls`** — derp requires TLS. Inspector has no TLS-required banner. `config_path` text input has no required marker or empty-value diagnostic. Default `listen: "127.0.0.1"` inappropriate for a server. `verify_client_url` / `mesh_with` / `stun` are `JsonField` only — no structured repeater. |
