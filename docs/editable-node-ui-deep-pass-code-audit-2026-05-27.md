...
| Node | Defect |
| --- | --- |
| rule-route-rule | `action === "route-options"` sub-form not implemented (no `override_address`/`override_port`/`network_strategy`/`fallback_*`/`udp_disable_domain_unmapping`/`tls_fragment`). Deprecated `geosite` / `source_geoip` / `geoip` still in `routeRuleAdvancedFields:397-399`. `action: "bypass"` not supported in outbound visibility branch (line 990 only matches "route"). Canvas outbound port not gated by action (see CC-6). `inbound` / `rule_set` references still free-text via `RuleListField`. |
| rule-dns-rule | **Phantom `dns-rule-action` Palette kind still present** (line 82). Missing 1.13 stable fields `interface_address`, `network_interface_address`, `default_interface_address`. `package_name_regex` (1.14 testing) absent. `evaluate` / `respond` / `route-options` action sub-forms not rendered. Canvas dns-server port not gated by action. |

### Rule-set

| Node | Defect |
| --- | --- |
| rule-set-local | `SbcNode.tsx:169` returns `download-detour` port for **all** rule-set kinds — inline and local should suppress it. Palette only ships one generic "Rule Set" entry that creates `remote`; no direct path to inline/local creation. |
...
| rule-set-inline | `useProjectStore.ts:578` `addRuleSet(config, "remote", ...)` hardcoded to remote — no inline creation path. Inline node still shows `download-detour` port. Canvas subtitle for inline still says generic "inline rule-set" with no rule count. |
...
| inbound-shadowsocks | shadowsocks entityType block (line 2806-2867) **only includes `method` + `plugin` selects**. `password`, `users[]`, `destinations[]`, `network`, `managed` all fall through to AdvancedScalarFields. `method` enum lists only 7 entries; official table has 13 (legacy stream ciphers missing). |
...
| service-derp | ✅ ~~scaffold returns 8 fields with no `tls`~~ — fixed 2026-05-27, scaffold now seeds `tls: { enabled: true, server_name: "" }`. `derp-service-needs-tls` diagnostic still fires for any imported derp without TLS. Inspector has no inline TLS-required banner (Inspector polish, not blocking). `config_path` still no required marker. Default `listen: "127.0.0.1"` still inappropriate for a server. `verify_client_url` / `mesh_with` / `stun` are `JsonField` only. |
